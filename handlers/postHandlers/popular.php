...
$_SESSION['jx']="998"; //1001 for latest Polls 1002 for upcoming polls 1003 for winners 1004 for latestPolls
...
else
{
	...
	else
	{
		...
		$getLatestPostsSQL="SELECT post.*, users.name, users.alias,users.userIdHash,users.gender FROM post INNER JOIN users ON post.userId=users.userId WHERE ((sharedWith REGEXP ? AND hiddenTo NOT REGEXP ? AND ( post.lifetime > ? OR isPermanent=1 ) ) OR post.userId = ?) AND displayStatus = 1";
		...
		$SQLEndPart="  ORDER BY popularityIndex DESC LIMIT 0,5";
