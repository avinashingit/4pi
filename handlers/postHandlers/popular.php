...
else
{
	...
	else
	{
		...
		$getLatestPostsSQL="SELECT post.*, users.name,users.userIdHash,users.gender FROM post INNER JOIN users ON post.userId=users.userId WHERE ((sharedWith REGEXP ? AND hiddenTo NOT REGEXP ? AND post.lifetime > ? ) OR post.userId = ?) AND displayStatus = 1";
