...
function aboutMe($userId,$mode,$isOwner)
{
	...
	if($mode == 1)
	{
		...
		$result1 = $conObj->fetchAll("SELECT users.gender,users.name,users.alias,users.userIdHash,IFNULL(experience.organisation,'') as organisation ,IFNULL(experience.designation,'') as designation ,IFNULL(about.dob,'') as dob ,IFNULL(about.description,'Some description about yourself.') as description,IFNULL(about.mailid,'') as mailid ,IFNULL(about.showMailId,'') as showMailId,IFNULL(about.address,'') as address,IFNULL(about.phone,'') as phone,IFNULL(about.showPhone,'') as showPhone,IFNULL(about.facebookId,'')as facebookId,IFNULL(about.googleId,'') as googleId,IFNULL(about.twitterId,'') as twitterId,IFNULL(about.linkedinId,'') as linkedinId ,IFNULL(about.pinterestId,'') as pinterestId  FROM users LEFT JOIN about ON users.userId=about.userId LEFT JOIN experience ON experience.userId=about.userId AND experience.experienceId=about.work WHERE users.userId = ?",$values1,false);
		// var_dump($result1);
		if($conObj->error == "")
		{
			...
			if($result1 != "")
			{
				...
				$obj = new about($result1['userIdHash'],$result1['name'],$result1['alias'],$result1['dob'],$result1['description'], $highestDegree,$work, $result1['mailid'],$result1['showMailId'],$result1['address'],explode(',',$result1['phone']),$result1['showPhone'],$result1['facebookId'],$result1['twitterId'],$result1['googleId'],$result1['linkedinId'],$result1['pinterestId'],$result1['gender'],$proPicExists,$isOwner,$resumeExists);
