...
else if($mode=4)
{
	...
	certifiedCoursesEdit($user,$_POST['_courseName'],$_POST['_duration'],$_POST['_institute']);
}
else if($mode=5)
{
	#experience Edit
	experienceEdit($user,$_POST['_company'],$_POST['duration'],$_POST['_role']);
}
else if($mode=6)
{
	#projects Edit
	projectEdit($user,$_POST['_projectTitle'],$_POST['_projectPosition'],$_POST['_duration'],$_POST['_teamMembers'],$_POST['_projectCompany']);
}
else if($mode=7)
{
	#workshop Edit
	workshopsEdit($user,$_POST['_workshopName'],$_POST['_duration'],$_POST['_location'],$_POST['_peopleAttended']);
}
else 
{
	# Erroneous Mode Sent
...
function experienceInsert($user,$organisation,$durationString,$title,$featuring,$experienceIdString)
{
	...
	if(($organisation != '') and ($title != '') and (($startDate["error_count"] == 0) and checkdate($startDate["month"], $startDate["day"], $startDate["year"])) and (($endDate["error_count"] == 0) and checkdate($endDate["month"], $endDate["day"], $endDate["year"])) and ($startDateTimestamp < $currentTimestamp) and ($endDateTimestamp < $currentTimestamp) and ($startDateTimestamp - $endDateTimestamp !=0) and($startDateTimestamp <=$endDateTimestamp))
		{
			...
			if($conObj->error == "")
			{
				...
				{
					notifyAdmin("suspicious attempt to change content in experience:".$courseId,$userId);
					echo 6;
					exit();
				}	
			}
...
function projectInsert($user,$title,$role,$durationString,$description,$teamMembers,$organisation,$projectIdString)
{
	...
	if(($title != '') and ($description != '') and ($role != '') and (($startDate["error_count"] == 0) and checkdate($startDate["month"], $startDate["day"], $startDate["year"])) and (($endDate["error_count"] == 0) and checkdate($endDate["month"], $endDate["day"], $endDate["year"])) and ($startDateTimestamp < $currentTimestamp) and ($endDateTimestamp < $currentTimestamp) and ($startDateTimestamp - $endDateTimestamp !=0)and($startDateTimestamp <= $endDateTimestamp))
		{
			...
			if($conObj->error == "")
			{
				...
				{
					notifyAdmin("suspicious attempt to change content in project:".$courseId,$userId);
					echo 6;
					exit();
				}	
			}
