			...
			if($mode == 1)
				{
					...
					$result1 = $conObj->fetchall("SELECT * FROM about WHERE uid = ?",$values1,false);
					
					if($conObj->error == "")
						{
							if($result1 != "")
								{
									$values2 = array(0 => array($userId => 's'));
									$result2 = $conObj->fetchall("SELECT name FROM users WHERE userId = ?",$values2,false);
									if($conObj->error == "")
										{
											if($result2 != "")
												{
												...
												{
													echo 'No values found for Query 2 in mode 1<br />';
												}
										}
									else
										{
									...
									$obj = new about($result1['propic'],$name,$result1['dob'],$result1['description'],$result1['resume'],$result1['hobbies'],$result1['mailid'],$result1['address'],$result1['phone'],$result1['city']);
									print_r($obj);
						...
						{	
							...
							while($achievements = $conObj->fetch($result1))
								{	
									...
									$obj = new achievements($achievements['competition'],$achievements['description'],$achievements['position'],$date1);
							...
							while($academics = $conObj->fetch($result1))
								{
									...
									$obj = new academics($degree,$academics['name'],$academics['start'],$academics['end'],$academics['cgpa']);
					...
					if($conObj->error == "")
						{
							...
							while($courses = $conObj->fetch($result1))
								{	
									$obj = new certifiedCourses($courses['title'],$courses['duration'],$courses['institutename']);
					...
					if($conObj->error == "")
						{
							...
							while($competitions = $conObj->fetch($result1))
								{
									$obj = new competitions($competitions['title'],$competitions['compdate'],$competitions['place'],$competitions['description']);
					...
					if($conObj->error == "")
						{
							while($experience = $conObj->fetch($result1))
								{
									$obj = new experience($experience['organisation'],$experience['start'],$experience['end'],$experience['title'],$experience['description']);
					...
					if($conObj->error == "")
						{
							...
							while($leaveMessage = $conObj->fetch($result1))
								{
									$obj = new leaveMessage($leaveMessage['name'],$leaveMessage['mailid'],$leaveMessage['message']);
					...
					if($conObj->error == "")
						{
							if($result1!="")
								{
									$obj = new objective($result1['description']);
									print_r($obj);
								}
					...
					if($conObj->error == "")
						{
							...
							while($projects = $conObj->fetch($result1))
								{
									$obj = new projects($projects['title'],$projects['role'],$projects['start'],$projects['end'],$projects['description']);
					...
					if($conObj->error == "")
						{
							...
							while($skillSets = $conObj->fetch($result1))
								{
									$obj = new skillSet($skillSets['skills'],$skillSets['rating']);
					...
					if($conObj->error == "")
						{
							if($result1!="")
								{
									$obj = new socialmedia($result1['facebookid'],$result1['googleid'],$result1['twitterid'],$result1['linkedinid']);
									print_r($obj);
								}
							else
								{
					...
					if($conObj->error == "")
						{
							...
							while($workshops = $conObj->fetch($result1))
								{
									$obj = new workshops($workshops['title'],$workshops['start'],$workshops['end'],$workshops['place'],$workshops['attendes']);
