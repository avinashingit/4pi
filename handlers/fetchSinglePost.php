...
else
{
	...
	else
	{
		...
		$getRequestedPostsSQL="SELECT post.*, users.name,users.userIdHash,users.gender,users.alias FROM post INNER JOIN users ON post.userId=users.userId WHERE sharedWith REGEXP ? AND postIdHash=? ";
